            ...
            <div className="container-custom">
              ...
              <div className="hidden md:block space-y-12">
                ...
                {news.filter(item => item.isPinned).length > 0 && (
                  <div>
                    <h3 className="text-xl font-serif text-accent-gold mb-6 flex items-center gap-2">
                       <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                       WICHTIG
                    </h3>
                    <div className="grid md:grid-cols-3 gap-8">
                      {news.filter(item => item.isPinned).slice(0, 3).map((item) => (
                ...
                {news.filter(item => !item.isPinned).length > 0 && (
                  <div>
                    <h3 className="text-xl font-serif text-wine-dark mb-6 flex items-center gap-2">
              ...
              <div className="md:hidden mb-8">
                {/* For mobile, we merge them but keep order: pinned first, then normal */}
                <NewsCarousel news={[
                  ...news.filter(item => item.isPinned).slice(0, 3),
                  ...news.filter(item => !item.isPinned).slice(0, 3)
                ]} />
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="p-6 space-y-4">
        ...
        {/* Read More */}
        {/* Read More or Reserve */}
        {news.event ? (
          <div className="inline-flex items-center gap-2 bg-wine text-white px-4 py-2 rounded-md font-medium group-hover:bg-wine-dark transition-all shadow-sm">
            Jetzt reservieren
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
