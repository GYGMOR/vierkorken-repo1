...
/**
 * Calculate cashback amount based on purchase and loyalty level
 */
export function calculateCashback(amount: number, loyaltyLevel: number): number {
  const level = LOYALTY_LEVELS.find((l) => l.level === loyaltyLevel);
  if (!level) return 0;

  return (amount * level.cashbackPercent) / 100;
}
